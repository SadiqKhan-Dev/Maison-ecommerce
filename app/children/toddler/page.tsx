...
export const metadata: Metadata = {
  title: "Toddler (2–5)",
  description:
    "Durable, playful clothing for toddlers. Built to move and made to last.",
};

export default async function ChildrenToddlerPage({
  searchParams,
}: {
  ...
  params.set("sub", "Toddler");
  const filters = parseFilters(params);

  const categoryProducts = mockProducts.filter(
    (p) => p.category === "CHILDREN" && p.subCategory === "Toddler"
  );
    ...
    <>
      <section className="relative h-[50vh] min-h-[360px] overflow-hidden bg-foreground">
        <div
          ...
          style={{
            backgroundImage:
              "url('https://images.unsplash.com/photo-1503944583220-79d8926ad5e2?auto=format&fit=crop&w=2000&q=80&seed=children-toddler')",
        ...
        <div className="relative h-full container-app flex flex-col justify-end pb-12 text-background">
          ...
          <p className="text-sm lg:text-base opacity-80 max-w-md">
            Durable, playful clothing built to move and made to last.
          </p>
        </div>
      </section>
      ...
      <Container className="py-10 lg:py-14">
        ...
        <SubCategoryNav category="CHILDREN" active="Toddler" />
        ...
        <div className="grid lg:grid-cols-[260px_1fr] gap-10 lg:gap-14">
          ...
          <div>
            ...
            {items.length === 0 ? (
              <EmptyState
                title="No toddler products in this view"
